...
describe("SimulationCard", () => {
  ...
  describe("Grid variant (default)", () => {
    ...
    it("should render grid variant when no variant is specified", () => {
      const { container } = render(
        <SimulationCard simulation={mockSimulation} {...mockHandlers} />
      );

      // Grid variant should have CardHeader
      expect(container.querySelector(".space-y-4")).toBeInTheDocument();
    });

    it("should render grid variant when explicitly specified", () => {
      const { container } = render(
        <SimulationCard simulation={mockSimulation} {...mockHandlers} variant="grid" />
      );
  ...
  describe("List variant", () => {
    it("should render list variant layout correctly", () => {
      const { container } = render(
        <SimulationCard simulation={mockSimulation} {...mockHandlers} variant="list" />
      );
      ...
      // List variant should have horizontal layout
      expect(container.querySelector(".flex.items-center.justify-between")).toBeInTheDocument();
    });

    it("should show same data in both variants", () => {
      const { rerender } = render(
        <SimulationCard simulation={mockSimulation} {...mockHandlers} variant="grid" />
      );

      // Check grid variant content
      expect(screen.getByText("Mi Startup de Ejemplo")).toBeInTheDocument();
      expect(screen.getByText("5000 €")).toBeInTheDocument();
      expect(screen.getByText("3.0")).toBeInTheDocument();
      expect(screen.getByText("Buena")).toBeInTheDocument();

      // Rerender with list variant
      rerender(<SimulationCard simulation={mockSimulation} {...mockHandlers} variant="list" />);

      // Check same content is still present
      expect(screen.getByText("Mi Startup de Ejemplo")).toBeInTheDocument();
      expect(screen.getByText("5000 €")).toBeInTheDocument();
      expect(screen.getByText("3.0")).toBeInTheDocument();
      expect(screen.getByText("Buena")).toBeInTheDocument();
    ...
    it("should call onDuplicate when duplicate button is clicked in list variant", () => {
      render(<SimulationCard simulation={mockSimulation} {...mockHandlers} variant="list" />);
    ...
    it("should call onDelete when delete button is clicked in list variant", () => {
      render(<SimulationCard simulation={mockSimulation} {...mockHandlers} variant="list" />);
    ...
    it("should disable buttons when loading in list variant", () => {
      render(
        <SimulationCard
